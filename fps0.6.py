...
class Enemy(Entity):
  ...
  def update(self):
    ...
    # ゆっくり近づく（ゾンビ感）
    if dist > 1.6:
      move = Vec3(to_player.x, 0, to_player.z).normalized()
      self.position += move * time.dt * self.speed

    # ふらつき（重要）
    self.rotation_z = sin(time.time() * 1.5) * 2
  ...
  def damage(self, amount):
    ...
    # ダメージ点滅
    self.animate_color(color.red, duration=0.05)
    invoke(self.animate_color, self.color, delay=0.05)
