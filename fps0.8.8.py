...
def update_minimap():
  ...
  # 敵（赤点）
  # =====================
  for pair in enemy_dots[:]:   # ← コピーで回す
    enemy, dot = pair

    # 敵が既に消えていたらミニマップから削除
    if not enemy or not enemy.enabled:
      destroy(dot)
      enemy_dots.remove(pair)
      continue
    ...
    dot.x = rx * MINIMAP_SCALE
    dot.y = rz * MINIMAP_SCALE

    dist = math.sqrt(rx * rx + rz * rz)
    dot.enabled = dist < (MINIMAP_RADIUS / MINIMAP_SCALE)
  ...
  # ミニマップ範囲外なら非表示
  if dist > (MINIMAP_RADIUS / MINIMAP_SCALE):
    exit_dot.enabled = False
    return

  exit_dot.enabled = True

  # 円の端でクランプ
  max_r = MINIMAP_RADIUS * 0.95
  x = rx * MINIMAP_SCALE
  y = rz * MINIMAP_SCALE
