...
class Enemy(Entity):
  ...
  def update(self):
    ...
    swing = sin(self.walk_time) * 20

    self.arm_l.rotation = (20, 0, 30)
    self.arm_r.rotation = (20, 0, -30)

    self.arm_l.rotation_x = 20 + swing
    self.arm_r.rotation_x = 20 - swing

    self.rotation_z = sin(self.walk_time * 0.7) * 2
    self.rotation_x = sin(self.walk_time * 0.4) * 1

    self.head.rotation_z = sin(self.walk_time * 1.3) * 4

  def damage(self, amount):
    self.hp -= amount

    self.blink(ucolor.rgb32(120, 0, 0))
    self.position -= self.forward * 0.3   # のけぞる
