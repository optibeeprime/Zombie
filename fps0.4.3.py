...
class Enemy(Entity):
  def __init__(self, **kwargs):
    super().__init__(parent=shootables_parent, model=None, collider=None, **kwargs)
  ...
  def update(self):
    ...
    if dist > 2:
      ...
      # 歩きモデルが消えていたら再表示
      if not self.model_entity.enabled:
        self.model_entity.enabled = True
    else:
      # 攻撃
      if self.model_entity.model != 'attack':
        destroy(self.model_entity)
        self.model_entity = Entity(
            parent=self,
            model='attack',   # attack.fbx
            scale=1,
            rotation=(0, 180, 0),
            y=0
        )
  ...
  def damage(self, amount):
    ...
    if self.hp <= 0:
      ...
      # 死亡アニメを再生後に消す
      invoke(lambda: destroy(death_entity), delay=2)
      spawn_enemy(1)
